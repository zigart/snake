	...
	private Direction moveToFruit(List<Point> snake, Point head, List<Point> obstacles, Point frutaMasCercana,
		...
		if (head.getX() > frutaMasCercana.getX()) {
			...
			if (previous.compatibleWith(Direction.LEFT) && !obstaculos[3] && !parteDeSnake[3] ) {
				...
				if (!obstaculos[2] && !parteDeSnake[2] && frutaMasCercana.getY() <= head.getY()) {
			...
			if (previous.compatibleWith(Direction.RIGHT) && !obstaculos[1] && !parteDeSnake[1]) {
				...
				if (!obstaculos[0] && !parteDeSnake[0] && previous.compatibleWith(Direction.UP)
						&& frutaMasCercana.getY() >= head.getY()) {
					return Direction.UP;
				} else if (!obstaculos[2] && !parteDeSnake[2]) {
					return Direction.DOWN;
				} else
					return Direction.LEFT;
			}
			...
			if (head.getY() > frutaMasCercana.getY()) {
				...
				if (previous.compatibleWith(Direction.DOWN) && !obstaculos[2] && !parteDeSnake[2]) {
					...
					if (!obstaculos[1] && !parteDeSnake[1] && previous.compatibleWith(Direction.RIGHT)
							&& frutaMasCercana.getX() >= head.getX()) {
						return Direction.RIGHT;
					} else if (!obstaculos[3] && !parteDeSnake[3]) {
						return Direction.LEFT;
					} else //ver si es elif en lugar de else. agregar condición en todos.
						return Direction.UP;
				}
				...
				if (previous.compatibleWith(Direction.UP) && !obstaculos[0] && !parteDeSnake[0]) {
					...
					if (!obstaculos[3] && !parteDeSnake[3] && previous.compatibleWith(Direction.LEFT)
							&& frutaMasCercana.getX() <= head.getX()) {
						// hay buscar la manera de que decida correctamente a donde
						return Direction.LEFT;
